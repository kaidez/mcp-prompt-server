...
async function readPromptHandler({ filename }: { filename: string }) {
...
async function savePromptHandler({ text }: { text: string }) {
  if (!text.trim()) {
    return {
...
describe("mcp-prompt-server", () => {
  ...
  it("list_prompts should return files when folder has content", async () => {
    sandbox.stub(fs, "readdirSync").returns(["test.txt", "notes.md"] as any);
  ...
  it("list_prompts should filter out non-.txt and non-.md files", async () => {
    sandbox
      .stub(fs, "readdirSync")
      .returns(["test.txt", "image.png", "notes.md", "data.json"] as any);
  ...
  it("read_prompt should return file contents", async () => {
    ...
    sandbox.stub(fs, "readFileSync").returns("What is integration engineering?" as any);
